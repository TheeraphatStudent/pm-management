...
public class Dashboard implements DashboardProps {
    ...
    private boolean preloadDashboard() throws FileNotFoundException {
        ...
        while (fr.hasNextLine()) {
            // 38 22 55 34 63 59 78 87 97 79 61 63 77 78 214 192 224 156 181 79
            // (ตัวอย่างไฟล์)
            String[] readLine = fr.nextLine().split("\\s+");
        ...
        if (dust >= 0 && dust <= 250) {
            ...
            btn.addActionListener(e -> {
                ...
                if (this.isActive) {
                    ...
                    this.parentPage.setStatisticData(dust, buttonPatentRate.get(btn),
                            buttonPeoples[currentRow][currentCol]);
